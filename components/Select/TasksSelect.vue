<template>
  <div
    ...
    ref="selectContainer"
  >
    <div class="selected" :class="{ open, isFull, isBlockchain }" @click="toggle">
      {{ reduceStringLength(selected.name, props.reduceLength) }}
      <span class="arrow-task" :class="{ flipped: open }"></span>
    </div>
    <div class="items" :class="{ selectHide: !open, isScroll }">
      <div v-if="filter" class="input-project-wrapper">
        <Icon class="search-icon" name="Search" :size="18" />
        <input @input="filterProjects($event)" type="text" :placeholder="filterPlaceholderValue" />
      </div>
...
const props = defineProps({
  ...
  filterPlaceholderValue: { type: String, default: 'Find a Project...' },
  reduceLength: {
    type: Number,
    required: false,
    default: 25,
...
.custom-select-task .items div:not(.empty-options, .input-project-wrapper) {
  display: flex;
